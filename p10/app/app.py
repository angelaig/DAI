...
def jquery():
    ...
    for p in pok:
        ...
        db.samples_pokemon.update({"_id" :p["_id"] },{'$set' : {"x": random.uniform(start_x, stop_x)}})
        db.samples_pokemon.update({"_id" :p["_id"] },{'$set' : {"y": random.uniform(start_y, stop_y)}})
        
        #db.samples_pokemon.update({'_id' : p["_id"],{'$set' : {'x' : random.uniform(start, stop) }})
